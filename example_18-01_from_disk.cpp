...
int main(int argc, const char *argv[]) {
  ...
  if (argc != 7) {
    cout << "\nERROR: Wrong number (" << argc -1 << ") of arguments, should be (6) input parameters\n";
    help(argv);
    return -1;
  }
  ...
  for(size_t i = 0; (i < filenames.size()) && (board_count < n_boards); ++i)
  {
    ...
    //
     if (found ) {
      ...
      cout << "Collected " << (int)image_points.size() << "total boards. This one from chessboard image #" << i << ", " << folder + filenames[i] << endl;
    }
